...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (orgDomains.length === 0) {
      return new Response(
        JSON.stringify({ error: 'No domains found for this organization' }),
        { status: 404, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    switch (exportType) {
      case 'reports': {
        // Build query for reports across all domains
        ...
        const filename = `${organization.slug}-all-domains-reports-${timestamp}.csv`;
        return createCSVResponse(csv, filename);
      }

      case 'sources': {
        // Build query for sources across all domains
        ...
        const filename = `${organization.slug}-all-domains-sources-${timestamp}.csv`;
        return createCSVResponse(csv, filename);
      }

      case 'timeline': {
        // Build query for timeline data across all domains
        const conditions = [inArray(reports.domainId, domainIds)];
        if (dateFromFilter) {
          conditions.push(gte(reports.dateRangeBegin, dateFromFilter));
        }
        ...
        const filename = `${organization.slug}-all-domains-timeline-${timestamp}.csv`;
        return createCSVResponse(csv, filename);
      }
